...
export default class ImageEditor {
    ...
    openEditor(imgId, imgSrc, closeCallback) {
        if (!this.editor) {
            this.editor = new Aviary.Feather({
                theme: 'light',
                language: this.locale,
                apiKey: config.aviaryApiKey,
                onLoad: () => this.onLoad(imgId, imgSrc, closeCallback)
            });
        } else {
            this.onLoad(imgId, imgSrc, closeCallback);
        }

    }

    onLoad(imgId, imgSrc, closeCallback) {
        this.editor.launch({
            image: imgId,
            url: imgSrc,
            onSave: (imageID, newURL) => {
                this.editor.close();
                closeCallback(imageID, newURL);
            },
            onError: (errorObj) => {
